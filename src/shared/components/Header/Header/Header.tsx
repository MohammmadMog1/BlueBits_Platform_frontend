...
export default function Header() {
	...
	return (
		<header className={`flex items-center justify-between px-4 lg:px-6 py-3.5 border-b flex-shrink-0
			 ${
			isDark
		...
		>
			<div className="flex items-center gap-3">
				...
				<div className={`hidden md:flex items-center gap-2.5 px-4 py-2.5 rounded-xl w-64 lg:w-72 transition-colors ${
					...
					<input type="text" placeholder="Search..."
						className={`bg-transparent text-sm w-full outline-none ${
							isDark ? 'text-white placeholder-gray-600' : 'text-gray-700 placeholder-gray-400'
						}`} />
				</div>
			</div>
			...
			<div className="flex items-center gap-2">
				...
				<button onClick={() => setTheme(isDark ? 'light' : 'dark')}
					className={`p-2.5 rounded-xl transition-all ${
						isDark ? 'bg-white/8 hover:bg-white/15 text-yellow-400' : 'bg-gray-100 hover:bg-gray-200 text-gray-500'
					}`}>
				...
				<button onClick={() => setNotifications(0)}
					className={`relative p-2.5 rounded-xl transition-all ${
						isDark ? 'bg-white/8 hover:bg-white/15 text-gray-400' : 'bg-gray-100 hover:bg-gray-200 text-gray-500'
					}`}>
