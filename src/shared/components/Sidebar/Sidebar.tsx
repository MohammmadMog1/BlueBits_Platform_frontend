...
export default function Sidebar({ navItems, userProfile }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* الفوتر وسيكشن الوصول السريع فوق اسم المستخدم */}
      <div
        className={`p-3 border-t overflow-hidden ${
          isDark ? "border-white/8" : "border-gray-100"
        }`}
      >
        {/* سيكشن الأزرار فوق اسم المستخدم */}
        <div className="mb-2.5 space-y-1">
          {!collapsed && (
            <p className={`px-3 text-[10px] font-black uppercase tracking-widest mb-2 ${isDark ? "text-gray-600" : "text-gray-400"}`}>
              {isAdminView ? "Admin Actions" : "Quick Actions"}
            </p>
          )}

          {isAdminView ? (
            /* بالنسبة للأدمن: زر الإعدادات + زر العودة لصفحات المستخدم */
            <>
              <button
              ...
              >
                ...
                {!collapsed && (
                  <span className="text-[13px] font-semibold">User View</span>
                )}
              </button>
            </>
          ) : (
            /* بالنسبة لليوزر: زر الانتقال للـ Admin Panel + زر الإعدادات */
            <>
              <button
        ...
        {/* بطاقة اسم المستخدم */}
        <div className="flex items-center p-2 rounded-xl transition-colors hover:bg-gray-50 dark:hover:bg-white/8">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#404293] to-[#2376BB] flex items-center justify-center flex-shrink-0 shadow-md">
          ...
          >
            ...
            <button
              onClick={() => logout()}
              title="Logout"
              className="p-1 rounded-lg hover:bg-gray-200/50 dark:hover:bg-white/10 transition-colors"
            >
              <LogOut
