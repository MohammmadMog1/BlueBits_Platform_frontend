...
function BottomBarLink({ item, active, isDark }: BottomBarLinkProps) {
    ...
    <>
      ...
      >
        {visibleItems.map((item) => (
          <BottomBarLink key={item.label} item={item} active={isActive(item.path)} isDark={isDark} />
        ))}
        ...
        {hasMore && (
          ...
          >
            <div className="relative p-1.5 rounded-xl">
              ...
              {switchAction && (
                <span className="absolute top-0.5 right-0.5 w-2 h-2 rounded-full bg-gradient-to-r from-[#404293] to-[#2376BB] ring-2 ring-white dark:ring-[#1a1b1e]" />
              )}
            </div>
