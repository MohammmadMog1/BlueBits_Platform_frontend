...
export default function ManagePermissionsModal({ user, onClose, onError }: ManagePermissionsModalProps) {
  ...
  const handleToggle = async (permission: Permission, hasPermission: boolean) => {
    ...
    try {
      ...
      onError("فشل تحديث الصلاحية. حاول مرة أخرى.");
    } finally {
      ...
      >
        <div className="px-7 pt-7 pb-5 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-gray-900 text-base">صلاحيات {user.name}</h3>
              <p className="text-xs text-gray-400 mt-0.5">تحديد الصلاحيات الإضافية الممنوحة للمستخدم</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-7 py-6 space-y-3">
          {PERMISSIONS_LIST.map((permission) => {
            const hasPermission = activePermissions.includes(permission.key);
            const isPending = pendingPermission === permission.key && isBusy;
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  disabled={isBusy}
                  onClick={() => handleToggle(permission.key, hasPermission)}
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400 pt-1">
            ...
            يتم حفظ كل تغيير فوراً على الخادم
          </div>
        </div>
