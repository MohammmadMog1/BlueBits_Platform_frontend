...
export function Hero({ isDark, navigate, stats }: HeroProps) {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16 sm:pt-20">
      ...
      <HexGrid className="absolute -right-10 top-0 w-[350px] sm:w-[600px] h-[350px] sm:h-[600px]" opacity={isDark ? 0.04 : 0.025} />
      <HexGrid className="absolute -left-20 bottom-0 w-[250px] sm:w-[400px] h-[250px] sm:h-[400px]" opacity={isDark ? 0.03 : 0.018} />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-16 py-16 sm:py-24">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5 max-w-3xl mx-auto">
            {stats.map(s => (
              <div key={s.label} className={`relative p-4 sm:p-6 rounded-2xl border group overflow-hidden transition-all duration-300 hover:-translate-y-1 ${
                isDark 
                ...
                <div className={`text-[11px] sm:text-xs font-semibold ${isDark ? 'text-gray-500' : 'text-slate-500'}`}>
                  {s.label}
                </div>
              </div>
            ))}
