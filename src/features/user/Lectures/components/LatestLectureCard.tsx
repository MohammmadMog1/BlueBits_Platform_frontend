...
export function LatestLectureCard({ lecture, onClick }: LatestLectureCardProps) {
  ...
  return (
    <button
      ...
      className={`snap-start flex-shrink-0 w-[160px] sm:w-[220px] rounded-xl sm:rounded-2xl border overflow-hidden text-left transition-all hover:-translate-y-1 hover:shadow-xl group backdrop-blur-md
    ...
    >
      ...
      >
        ...
        <img
          src={Logo}
          alt="BlueBits"
          className="absolute top-1.5 right-1.5 w-4 h-4 sm:top-2 sm:right-2 sm:w-6 sm:h-6 rounded-md object-contain bg-white/90 p-0.5 shadow-sm"
        />

        {lectureNumber && (
          <span
            className={`absolute bottom-1.5 left-1.5 px-1.5 py-0.5 sm:bottom-2 sm:left-2 sm:px-2 rounded-md text-[9px] sm:text-[10px] font-bold ${isDark ? "bg-black/50 text-white" : "bg-white/90 text-[#404293]"}`}
          >
            #{lectureNumber}
          </span>
        )}
